...
main (int argc, char *argv[])
{
 ...
 Ipv4AddressHelper address1;
  ...
  //std::cout<<"Address: "<< address.devices1<<std::endl;

  UdpEchoClientHelper echoClient1 (interfaces3.GetAddress (1), 50);
  echoClient1.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClient1.SetAttribute ("Interval", TimeValue (Seconds (0.01)));
  echoClient1.SetAttribute ("PacketSize", UintegerValue (1048));
...
Ipv4GlobalRoutingHelper::PopulateRoutingTables();
  ...
  UdpEchoServerHelper echoServer1 (51);
  ApplicationContainer serverApps1 = echoServer1.Install (nodes.Get (2));
  serverApps1.Start (Seconds (1.0));
  serverApps1.Stop (Seconds (10.0));
  UdpEchoClientHelper echoClient2 (interfaces2.GetAddress (1), 51);
...
anim.SetConstantPosition (nodes.Get(3), 200.0, 100.0);
