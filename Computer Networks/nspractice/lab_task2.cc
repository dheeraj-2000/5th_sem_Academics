...
main (int argc, char *argv[])
{
  ...
  PacketSinkHelper sink1 ("ns3::TcpSocketFactory",InetSocketAddress (Ipv4Address::GetAny (),9));
  ApplicationContainer sink1Apps = sink1.Install (p2pNodes.Get (1));
  sink1Apps.Start (Seconds (0.0));
  sink1Apps.Stop (Seconds (10.0));

  PacketSinkHelper sink ("ns3::TcpSocketFactory",InetSocketAddress (Ipv4Address::GetAny (),10));
  ApplicationContainer sinkApps = sink.Install (p2pNodes.Get (1));
  sinkApps.Start (Seconds (0.0));
  sinkApps.Stop (Seconds (10.0));
  PacketSinkHelper sink2 ("ns3::TcpSocketFactory",InetSocketAddress (Ipv4Address::GetAny (),11));
  ApplicationContainer sink2Apps = sink2.Install (p2pNodes.Get (1));
  sink2Apps.Start (Seconds (0.0));
  sink2Apps.Stop (Seconds (10.0));
  PacketSinkHelper sink3 ("ns3::TcpSocketFactory",InetSocketAddress (Ipv4Address::GetAny (),12));
  ApplicationContainer sink3Apps = sink3.Install (p2pNodes.Get (1));
  sink3Apps.Start (Seconds (0.0));
  sink3Apps.Stop (Seconds (10.0));
